...
import cv2
class KMeans():
    def __init__(self, n_clusters, boxes, data_size, label_file, max_epoch=1000, cls=''):
    ...
    def get_silhouette_coefficient(self, centroids, boxes):
        ...
        for label, wh in boxes:
            if boxes_in_class[label]:
                if len(boxes_in_class[label])<2000:
...
if __name__ == '__main__':
    ...
    if mode == 'person':
        k_and_c = load_centroids('results/large_box_by_k_means_both_new_k_d_person.txt')
        cls = '0'
        box, data_size = dataloader_by_class_new(label_txt, cls=cls)
    elif mode == 'face':
        k_and_c = load_centroids('results/large_box_by_k_means_both_new_k_d_face.txt')
        cls = '1'
        box, data_size = dataloader_by_class_new(label_txt,cls= cls)
    else:
        k_and_c = load_centroids('results/large_box_by_k_means_both_new_k_d.txt')
        cls = ''
    ...
    for k in k_and_c:
        ks.append(k)
        print(k)
        kmeans = KMeans(k, box, data_size, label_txt, cls=cls)
