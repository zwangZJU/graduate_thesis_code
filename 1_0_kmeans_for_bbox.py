...
if __name__ == '__main__':
    ...
    for k in range(5, 6, 1):
        kmeans = KMeans(k, label_txt, cls='1', file_name='large_box_by_k_means_face_new')

        result = kmeans.kmeans(square=True)
    # large person
    for k in range(5, 6, 1):
        kmeans = KMeans(k, label_txt, cls='0', file_name='large_box_by_k_means_person_new')
        result = kmeans.kmeans(square=True)

    # both
    for k in range(5, 6, 1):
        kmeans = KMeans(k, label_txt, cls='', file_name='large_box_by_k_means_both_new')
        result = kmeans.kmeans(square=True)
