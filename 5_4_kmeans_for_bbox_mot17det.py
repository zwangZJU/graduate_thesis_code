...
def dataloader_by_class_new( path, size=640, cls='0'):
    '''
    将label的宽高值读入，path中存储着所有label的路径，每个label中的每一行是calss x, y, w, h
    为了减少内存的使用，只载入w和h，并将比例换算成实际长度
    :param path: 保存所有label路径的txt文件
    ...
    with open(path, 'r') as flist:
        for line in flist.readlines():
            img_path = line.replace('labels', 'img1').replace('.txt', '.jpg').strip()
            ...
            with open(line.strip(), 'r') as f:
                for info in f.readlines():
                    ...
                    w, h = info.split()[-2:]
                    if w0 > h0:
                        wh = [int(float(w) * size), int(float(h) * h0 * size / w0)]
                    else:
                        wh = [int(float(w) * w0 * size / h0), int(float(h) * size)]
                    all_wh.append([label, wh])
                    data_size += 1
    print("[*]Load data successfully!")
    print("There are {} boxes".format(data_size))
...
if __name__ == '__main__':
    ...
    # print(1, image)
    label_txt = 'E:/Datasets/MOT17Det/train/train_labels.txt'
    ...
    kmeans = KMeans(9, box, data_size, cls='0', file_name='mot17det_bbox')
